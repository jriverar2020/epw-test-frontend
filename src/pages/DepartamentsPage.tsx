...
import { departamentsApi, type Departament } from "../api/departaments";
import { useDebouncedValue } from "../hooks/useDebouncedValue";

export default function DepartamentsPage() {

    const [rows, setRows] = useState<Departament[]>([]);
    ...
    useEffect(() => {
        (async () => {
            ...
            setErr(null);
            try {
                const data = await departamentsApi.list();
                setRows(data);
            } catch (e) {
                setErr(String(e));
            } finally {
        ...
        <div className="min-h-screen bg-slate-50">
            <header className="border-b bg-white">
                <div className="mx-auto max-w-5xl px-4 py-4 flex items-center justify-between">
                    <h1 className="text-xl font-semibold">MiniPOS — Departaments</h1>
                    <span className="text-sm text-slate-500">Backend: /departments</span>
                </div>
            </header>
